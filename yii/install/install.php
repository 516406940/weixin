...
if($action == 'db') {
    if($ispost) {
        ...
        if(file_exists(IA_DIR . 'install/wei.sql')){
            $sql = file_get_contents(IA_DIR. '/install/wei.sql');
            if(empty($sql)) {
        ...
        if (empty($error_msg)) {
            ...
            mysql_query("INSERT INTO {$dbprefix}user (user_name, user_pwd) VALUES('{$adminuser}', '{$adminpwd}')");
...
<div class="header">
	<div class="top">
		<h1 style="color:#ffffff;text-align:center;margin-top:20px;">微信管理系统安装程序<sup>　2016</sup></h1>
	</div>
</div>
<div class="main">
	<div class="pleft">
...
function tpl_install_license() {
    echo <<<EOF
		...
		<div class="pr-agreement">
				<p>版权所有 (c)2016，微信开发团队保留所有权利。 </p>
				<p>感谢您选择OneTeam微信管理系统（以下简称WE，WE基于 PHP + MySQL的技术开发，全部源码开放。</p>
